...
function AppointmentForm({ onSubmit, onCancel, initialServiceId }: Props) {
  ...
  // Only show staff who are assigned to the selected service
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!time) { toastError("Please select a time slot."); return; }
    if (!customerName.trim()) { toastError("Please enter your full name."); return; }
    if (!customerPhone.trim()) { toastError("Please enter your phone number."); return; }
  ...
  const selectedStaffObj = staffList.find(s => String(s.id) === staffId);
  ...
  return (
    <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 max-w-2xl mx-auto">
      <h2 className="text-2xl font-bold text-gray-800 mb-1">Book Appointment</h2>
      <p className="text-gray-500 text-sm mb-6">Choose your service, stylist, and a time that works for you.</p>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="flex flex-col">
            <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">Service</label>
            <select
              value={serviceId}
            ...
            >
              {services.map(s => (
                <option key={s.id} value={s.id}>{s.name} — ${s.price} · {s.durationMinutes ?? s.duration} min</option>
              ))}
            </select>
          </div>

          <div className="flex flex-col">
            <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">Stylist</label>
            <select
              value={staffId}
            ...
            {eligibleStaff.length === 0 && (
              <p className="text-xs text-amber-600 mt-1">No staff assigned to this service yet.</p>
            )}
          </div>
        </div>
        ...
        <div className="flex flex-col max-w-xs">
          <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">Date</label>
          <input
            type="date"
            value={date}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-gray-100 pt-4">
          <div className="flex flex-col">
            <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">Full Name <span className="text-red-500">*</span></label>
            <input
              type="text"
              value={customerName}
          ...
          <div className="flex flex-col">
            <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">Phone Number <span className="text-red-500">*</span></label>
            <input
              type="tel"
              value={customerPhone}
              ...
              {availableSlots.map(slot => {
                ...
                return (
                  <button
                    ...
                    className={`flex flex-col items-center py-2 px-2 rounded-xl border text-xs font-medium transition-all duration-150 ${
                      isSelected
                  ...
                  >
                    ...
                    {slotEnd && <span className={`text-xs mt-0.5 ${isSelected ? "text-pink-100" : "text-gray-400"}`}>→ {to12h(slotEnd)}</span>}
                  </button>
                );
              })}
            </div>
        ...
        {time && selectedService && (
          <div className="bg-gradient-to-br from-pink-50 to-pink-100/50 p-4 rounded-xl border border-pink-200">
            <h3 className="font-bold text-pink-800 mb-3 flex items-center gap-1.5">
              <span>✓</span> Appointment Summary
            </h3>
            <div className="grid grid-cols-2 gap-x-6 gap-y-1.5 text-sm">
              <div><span className="text-gray-500">Service</span><br/><span className="font-semibold text-gray-800">{selectedService.name}</span></div>
              <div><span className="text-gray-500">Price</span><br/><span className="font-semibold text-gray-800">${selectedService.price}</span></div>
              <div><span className="text-gray-500">Stylist</span><br/><span className="font-semibold text-gray-800">{staffId === "any" ? "Any Available" : `${selectedStaffObj?.user?.firstName ?? selectedStaffObj?.firstName} ${selectedStaffObj?.user?.lastName ?? selectedStaffObj?.lastName}`}</span></div>
              <div><span className="text-gray-500">Duration</span><br/><span className="font-semibold text-gray-800">{duration} minutes</span></div>
              <div><span className="text-gray-500">Date</span><br/><span className="font-semibold text-gray-800">{date}</span></div>
              <div>
              ...
              <div className="col-span-2 mt-2 pt-2 border-t border-pink-100">
                <span className="text-gray-500">Customer Details</span><br/>
        ...
        <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
          ...
          <Button type="submit" disabled={!time || loadingSlots}>
            {loadingSlots ? "Checking availability..." : "Confirm Booking"}
          </Button>
        </div>
      </form>
    </div>
